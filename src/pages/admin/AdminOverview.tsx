...
const AdminOverview = () => {
  ...
  const { data: workshopParticipation } = useQuery({
    queryKey: ['admin-workshop-participation'],
    queryFn: async () => {
      ...
      const participationPromises = workshops.map(async (workshop) => {
        const { count } = await supabase
          .from('user_workshops')
          .select('*', { count: 'exact', head: true })
        ...
        return {
          name: workshop.title.length > 15 ? workshop.title.substring(0, 15) + '...' : workshop.title,
          value: count || 0
        };
      });

      const participation = await Promise.all(participationPromises);
      return participation.filter(e => e.value > 0).slice(0, 5);
    }
  });
  ...
  const workshopParticipationData = workshopParticipation || [];
  ...
  return <div className="p-4 md:p-6 lg:p-8 space-y-8">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Workshop Participation Distribution</CardTitle>
            <CardDescription>Popular workshops breakdown</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={workshopParticipationData} cx="50%" cy="50%" labelLine={false} label={({
                name,
                percent
              }) => `${name}: ${(percent * 100).toFixed(0)}%`} outerRadius={80} fill="#8884d8" dataKey="value">
                  {workshopParticipationData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                </Pie>
                <Tooltip contentStyle={{
                backgroundColor: "hsl(var(--card))",
                border: "1px solid hsl(var(--border))",
